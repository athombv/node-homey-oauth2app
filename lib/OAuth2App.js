...
module.exports = class OAuth2App extends Homey.App {
  ...
  setOAuth2Config({
    ...
    if( allowMultiSession === true && Homey.version.charAt(0) === '1' )
  ...
  deleteOAuth2Client({
    ...
    // remove from storage
    const savedClients = this.getSavedOAuth2Clients();
    delete savedClients[sessionId];
    Homey.ManagerSettings.set(SETTINGS_KEY, savedClients); 
  ...
  getOAuth2Client({
    ...
    const savedClients = this.getSavedOAuth2Clients();
    if( savedClients && savedClients[sessionId]) {
      const {
        token,
        title,
      } = savedClients[sessionId];
