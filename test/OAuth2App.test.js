...
test('test', async function() {
  ...
  fetch.mockReturnValue(Promise.resolve(new Response("{\"access_token\": \"access123\", \"refresh_token\": \"refresh123\"}", {headers: {
    'content-type': 'application/json'
  }})));

  await client.getTokenByCredentials({username: "username", password: "password"});
  fetch.mockReturnValue(Promise.resolve(new Response("[{\"light\": \"wow\"}]", {headers: {
    'content-type': 'application/json'
  }})));
